...
public class UserPreferencesConfigurator implements NamespaceConfigurator {
  ...
  private Secret preparePreferencesSecret(NamespaceResolutionContext namespaceResolutionContext)
    ...
    //      throw new InfrastructureException(
    //          String.format(
    //              "Preferences of user with id:%s are empty. Cannot create user preferences
    // secrets.",
    //              namespaceResolutionContext.getUserId()));
    //    }
    ...
    //        .addToAnnotations(DEV_WORKSPACE_MOUNT_PATH_ANNOTATION,
    // USER_PREFERENCES_SECRET_MOUNT_PATH)
    //        .endMetadata()
    //        .build();
    return null;
  }
